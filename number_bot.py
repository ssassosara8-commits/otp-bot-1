...
OTP_QUEUE_LOCK_FILE = "otp_queue.json.lock"
...
APPROVAL_SYSTEM_FILE = "approval_system.json"
...
def init_files():
    ...
    if not os.path.exists(OTP_STATS_FILE):
        with open(OTP_STATS_FILE, "w") as f:
            json.dump({}, f)
    
    if not os.path.exists(APPROVAL_SYSTEM_FILE):
        with open(APPROVAL_SYSTEM_FILE, "w") as f:
...
# ======== OTP Statistics Functions ========

def increment_otp_count(user_id):
    """Increment OTP count for a user"""
    try:
        stats = load_json(OTP_STATS_FILE)
        user_key = str(user_id)
        ...
        stats[user_key] += 1
        save_json(OTP_STATS_FILE, stats)
        return stats[user_key]
    except Exception as e:
        print(f"⚠️ Error incrementing OTP count: {e}")
        return 0

def get_otp_stats():
    """Get all OTP statistics"""
    try:
        return load_json(OTP_STATS_FILE)
    except:
        return {}

def handle_otp_stats(chat_id):
    """Display OTP statistics for all users"""
    stats = get_otp_stats()
    assignments = load_json(USER_ASSIGNMENTS_FILE)
    
    if not stats:
        send_message(chat_id, "📊 <b>OTP Statistics</b>\n\n⚠️ No OTP data yet.")
        return
    
    # Sort by OTP count (descending)
    sorted_stats = sorted(stats.items(), key=lambda x: x[1], reverse=True)
    
    msg = "📈 <b>OTP Statistics</b>\n\n"
    msg += "👥 <b>User OTP Counts:</b>\n\n"
    ...
    for idx, (user_id, count) in enumerate(sorted_stats, 1):
        ...
        if user_id in assignments:
            country = assignments[user_id].get("country", "Unknown")
            number = assignments[user_id].get("number", "N/A")
            user_info = f" ({country} - {number})"
        
        msg += f"{idx}. User <code>{user_id}</code>{user_info}\n"
        msg += f"   🔑 OTPs Received: <b>{count}</b>\n\n"
        total_otps += count
    
    msg += f"📊 <b>Total OTPs Sent:</b> {total_otps}\n"
    msg += f"👥 <b>Total Users:</b> {len(stats)}"
...
def is_approval_system_enabled():
    ...
    try:
        settings = load_json(APPROVAL_SYSTEM_FILE)
        return settings.get("enabled", True)
    except:
        return True

def toggle_approval_system():
    """Toggle approval system on/off"""
    try:
        settings = load_json(APPROVAL_SYSTEM_FILE)
        current = settings.get("enabled", True)
        settings["enabled"] = not current
        save_json(APPROVAL_SYSTEM_FILE, settings)
        return settings["enabled"]
    except Exception as e:
        print(f"⚠️ Error toggling approval system: {e}")
        return True

def handle_approval_system_toggle(chat_id):
    """Handle approval system toggle button"""
    enabled = toggle_approval_system()
    
    if enabled:
        msg = "🔓 <b>Approval System: ENABLED</b>\n\n"
        msg += "✅ New users must be approved by admin to use the bot.\n"
        msg += "📝 Pending requests will appear in 'Manage Members'."
    else:
        msg = "🔒 <b>Approval System: DISABLED</b>\n\n"
        msg += "⚡ All users can use the bot immediately without approval.\n"
        msg += "⚠️ Anyone can request numbers freely."
    
    keyboard = {
        "inline_keyboard": [[
            {"text": "🔄 Toggle Again", "callback_data": "toggle_approval"}
        ]]
    }
...
def view_accounts(chat_id):
    """Display all accounts to admin"""
    accounts = load_accounts()
    
    if not accounts:
        send_message(chat_id, "⚠️ No accounts added yet.\n\nUse the button below to add your first account.", 
                    reply_markup={
                        "inline_keyboard": [[
    ...
    msg = "🔑 <b>Panel Accounts:</b>\n\n"
    for idx, acc in enumerate(accounts, 1):
        status = "✅ Active" if acc.get("enabled", True) else "❌ Disabled"
        msg += f"<b>{idx}. {acc['name']}</b>\n"
        msg += f"   👤 Username: <code>{acc['username']}</code>\n"
        msg += f"   🔒 Password: <code>{'*' * len(acc['password'])}</code>\n"
        msg += f"   📊 Status: {status}\n\n"
    
    keyboard = {
        "inline_keyboard": [
            [{"text": "➕ Add Account", "callback_data": "add_account"}],
            [{"text": "🗑️ Delete Account", "callback_data": "delete_account_menu"}],
            [{"text": "🔄 Toggle Status", "callback_data": "toggle_account_menu"}]
        ]
    }
    
    send_message(chat_id, msg, reply_markup=keyboard)

def show_delete_account_menu(chat_id):
    """Show inline keyboard to select account to delete"""
    accounts = load_accounts()
    
    if not accounts:
        send_message(chat_id, "⚠️ No accounts to delete.")
        return
    
    keyboard = {"inline_keyboard": []}
    for idx, acc in enumerate(accounts):
        keyboard["inline_keyboard"].append([
            {"text": f"🗑️ {acc['name']} ({acc['username']})", "callback_data": f"del_acc_{idx}"}
        ])
    keyboard["inline_keyboard"].append([
        {"text": "« Back", "callback_data": "back_to_accounts"}
    ])
    
    send_message(chat_id, "🗑️ <b>Select account to delete:</b>", reply_markup=keyboard)

def show_toggle_account_menu(chat_id):
    """Show inline keyboard to toggle account status"""
    accounts = load_accounts()
    
    if not accounts:
        send_message(chat_id, "⚠️ No accounts to toggle.")
        return
    
    keyboard = {"inline_keyboard": []}
    for idx, acc in enumerate(accounts):
        status_icon = "✅" if acc.get("enabled", True) else "❌"
        keyboard["inline_keyboard"].append([
            {"text": f"{status_icon} {acc['name']} ({acc['username']})", "callback_data": f"toggle_acc_{idx}"}
        ])
    keyboard["inline_keyboard"].append([
        {"text": "« Back", "callback_data": "back_to_accounts"}
    ])
    
    send_message(chat_id, "🔄 <b>Select account to enable/disable:</b>", reply_markup=keyboard)
...
def delete_account(index):
    ...
    if 0 <= index < len(accounts):
        ...
        print(f"🗑️ Deleted account: {deleted['username']}")
        return True
    return False

def toggle_account(index):
    """Toggle account enabled/disabled status"""
    accounts = load_accounts()
    if 0 <= index < len(accounts):
        accounts[index]["enabled"] = not accounts[index].get("enabled", True)
        save_accounts(accounts)
        status = "enabled" if accounts[index]["enabled"] else "disabled"
        print(f"🔄 Account {accounts[index]['username']} {status}")
        return True, accounts[index]["enabled"]
    return False, None
...
def get_admin_menu():
    ...
    return {
        "keyboard": [
            ...
            ["📈 OTP Stats", "🔓 Approval System"],
            ["👥 Active Users", "📢 Broadcast"],
            ["🔐 Manage Members", "🔑 Manage Accounts"]
        ],
...
def handle_callback(callback_query):
    ...
    if data == "toggle_approval" and user_id == ADMIN_USER_ID:
        enabled = toggle_approval_system()
        ...
        answer_callback(query_id, "✅ Setting updated!")
        return
    ...
    # ===== Account Management Callbacks (Admin Only) =====
    
    # Add account
    if data == "add_account" and user_id == ADMIN_USER_ID:
        admin_states[user_id] = {"action": "add_account", "step": 1}
        send_message(chat_id, "📝 <b>Add New Account</b>\n\n<b>Step 1/3:</b> Enter a name for this account (e.g., Main Panel, Second Account, etc.):")
        answer_callback(query_id)
        return
    
    # Delete account menu
    if data == "delete_account_menu" and user_id == ADMIN_USER_ID:
        show_delete_account_menu(chat_id)
        answer_callback(query_id)
        return
    
    # Delete specific account
    if data.startswith("del_acc_") and user_id == ADMIN_USER_ID:
        idx = int(data.replace("del_acc_", ""))
        if delete_account(idx):
            send_message(chat_id, "✅ Account deleted successfully!")
            view_accounts(chat_id)
        else:
            send_message(chat_id, "❌ Failed to delete account!")
        answer_callback(query_id, "🗑️ Account deleted!")
        return
    
    # Toggle account menu
    if data == "toggle_account_menu" and user_id == ADMIN_USER_ID:
        show_toggle_account_menu(chat_id)
        answer_callback(query_id)
        return
    
    # Toggle specific account
    if data.startswith("toggle_acc_") and user_id == ADMIN_USER_ID:
        idx = int(data.replace("toggle_acc_", ""))
        success, enabled = toggle_account(idx)
        if success:
            status = "enabled ✅" if enabled else "disabled ❌"
            send_message(chat_id, f"✅ Account {status}!")
            view_accounts(chat_id)
            answer_callback(query_id, f"🔄 Account {status}!")
        else:
            answer_callback(query_id, "❌ Failed to toggle account!")
        return
    
    # Back to accounts
    if data == "back_to_accounts" and user_id == ADMIN_USER_ID:
        view_accounts(chat_id)
        answer_callback(query_id)
        return
    
    # ===== End Account Management Callbacks =====
...
def monitor_otp_queue():
    ...
    while True:
        try:
            ...
            # Read new OTP entries (Inter-process safe)
            lock = FileLock(OTP_QUEUE_LOCK_FILE)
            with lock:
                with open(OTP_QUEUE_FILE, "r", encoding="utf-8") as f:
                    lines = f.readlines()
                    ...
                    try:
                        ...
                        for user_id, assignment in assignments.items():
                            if assignment["number"] == number:
                                ...
                                # Increment OTP count for statistics
        ...
        if "message" in update:
            ...
            if user_id == ADMIN_USER_ID:
                if text.startswith("/addcountry"):
                    ...
                    handle_otp_stats(chat_id)
                    return
                elif text == "🔓 Approval System":
                    handle_approval_system_toggle(chat_id)
                    return
                
                # Handle add account state
                if user_id in admin_states and admin_states[user_id].get("action") == "add_account":
                    step = admin_states[user_id].get("step", 1)
                    
                    if step == 1:
                        admin_states[user_id]["name"] = text
                        admin_states[user_id]["step"] = 2
                        send_message(chat_id, "👤 <b>Enter Username:</b>\n\nThis is the username you use to login to the panel.")
                        return
                    elif step == 2:
                        admin_states[user_id]["username"] = text
                        admin_states[user_id]["step"] = 3
                        send_message(chat_id, "🔒 <b>Enter Password:</b>\n\nThis is the password for the panel account.")
                        return
                    elif step == 3:
                        password = text
                        name = admin_states[user_id]["name"]
                        username = admin_states[user_id]["username"]
                        
                        success, msg = add_account(name, username, password)
                        
                        if success:
                            send_message(chat_id, f"✅ <b>Account Added Successfully!</b>\n\n📝 Name: {name}\n👤 Username: {username}\n🔒 Password: {'*' * len(password)}")
                        else:
                            send_message(chat_id, f"❌ <b>Failed to add account:</b>\n{msg}")
